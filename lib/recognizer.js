...
function mapTextAnnotations(res) {
  ...
  for (i = 0; i < res.textAnnotations.length; i++) {
    var text = res.textAnnotations[i];
...
/**
 * Maps logoAnnotation response to internal format.
 *
 * @method     mapLogoAnnotations
 * @param      {Object}  res     Google vision API response.
 * @return     {Array}   Array of recognized logos and its coordinates.
 */
function mapLogoAnnotations(res) {
  if (!res || !res.logoAnnotations) {
    return [];
  }

  var i, result = [];
  for (i = 0; i < res.logoAnnotations.length; i++) {
    var logo = res.logoAnnotations[i];
    result.push({
